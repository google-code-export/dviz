...
GroupPlayerEditor::GroupPlayerEditor(SlideGroup *g, QWidget *parent) : 
	...
	m_syncToDatabase(true)
{
	...
	QHBoxLayout * cols = new QHBoxLayout(centerWidget);
...
void GroupPlayerEditor::showSyncOption(bool flag)
{
	m_syncBox->setVisible(flag);
}

void GroupPlayerEditor::editSongTemplate()
{
	if(!m_slideGroup)
		return;
...
void GroupPlayerEditor::editorWindowClosed()
{
	SongSlideGroup * songGroup = dynamic_cast<SongSlideGroup*>(m_slideGroup);
	if(!songGroup)
		return;
...
GroupPlayerEditor::~GroupPlayerEditor() 
{
	if(m_editWin)
		delete m_editWin;
}

void GroupPlayerEditor::setSlideGroup(SlideGroup *g,Slide */*curSlide*/) 
{
	m_slideGroup = g;
	SongSlideGroup * songGroup = dynamic_cast<SongSlideGroup*>(m_slideGroup);
	if(!songGroup)
...
void GroupPlayerEditor::setSlideGroup(SlideGroup *g,bool syncToDatabase) 
{
	setSlideGroup(g);
	if(syncToDatabase)
	{
...
void GroupPlayerEditor::setSyncToDatabase(bool sync)
{
	m_syncToDatabase = sync;
}

void GroupPlayerEditor::accepted()
{
	if(m_slideGroup)
	{
		SongSlideGroup * songGroup = dynamic_cast<SongSlideGroup*>(m_slideGroup);
...
void GroupPlayerEditor::closeEvent(QCloseEvent *evt)
{
// 	if(m_syncToDatabase)
// 	{
