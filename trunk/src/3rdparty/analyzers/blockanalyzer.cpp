...
{
    changeTimeout( 20 ); //Amarok::config( "General" ).readEntry( "Timeout", 20 ) );
...
BlockAnalyzer::paletteChange( const QPalette& ) //virtual
{
   ...
   const QColor fg = ensureContrast( bg , bg ); //, KGlobalSettings::activeTitleColor() );
...
BlockAnalyzer::contextMenuEvent( QContextMenuEvent * )
{
    ...
    menu->setTitle( tr( "Framerate" ) );

    QAction *a = menu->addAction( tr("50 fps"), this, SLOT( set50fps() ) );
